    ...
    <div style={{ minHeight: '100vh', background: BEIGE }}>
      ...
      <div style={{ background: BEIGE_CARD, borderBottom: `2px solid ${BEIGE_BORDER}`, padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        ...
        <button onClick={() => router.push('/dashboard')}
          ...
          style={btn('back', { padding: '8px 16px', borderRadius: 100, border: `2px solid ${BEIGE_BORDER}`, background: BEIGE_CARD, fontSize: 13, fontWeight: 700, color: TEXT_MUTED, fontFamily: 'inherit' }, { borderColor: PRIMARY, color: PRIMARY, background: PRIMARY_BG })}>
          ← Dashboard
        </button>
      </div>
      ...
      <div style={{ maxWidth: 600, margin: '0 auto', padding: '24px 16px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
          {children.map(child => (
            <button key={child.id} onClick={() => { setSelected(selected?.id === child.id ? null : child); setAdding(false) }}
              ...
              <div style={{ width: 44, height: 44, borderRadius: '50%', background: AVATAR_COLORS[child.color_index % AVATAR_COLORS.length], display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontSize: 15, flexShrink: 0 }}>
                {child.name.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()}
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 700, fontSize: 15, color: TEXT }}>{child.name}</div>
                <div style={{ fontSize: 12, color: TEXT_MUTED }}>{child.age_group} · {child.city}</div>
              </div>
              <span style={{ fontSize: 18, color: TEXT_MUTED }}>{selected?.id === child.id ? '▲' : '▼'}</span>
            </button>
          ))}
        ...
        {selected && !adding && (
          <div style={{ background: BEIGE_CARD, borderRadius: 20, overflow: 'hidden', border: `2px solid ${BEIGE_BORDER}`, boxShadow: '0 2px 12px rgba(0,0,0,0.06)', marginBottom: 16, animation: 'fadeIn 0.2s ease' }}>
            <div style={{ background: `linear-gradient(135deg, ${PRIMARY}, ${GREEN})`, padding: '24px 20px', color: 'white' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                ...
                <div>
                  ...
                  <p style={{ margin: '4px 0 0', opacity: 0.85, fontSize: 13 }}>{selected.age_group} · {selected.city}, {selected.country} · {selected.learn_style}</p>
                </div>
              </div>
            </div>
            ...
            <div style={{ padding: 20 }}>
              {selected.subjects?.length > 0 && (
                <div style={{ marginBottom: 16 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: TEXT_MUTED, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>Interests</div>
              ...
              {selected.notes && (
                <div style={{ background: BEIGE, borderRadius: 12, padding: 14, marginBottom: 16, border: `1px solid ${BEIGE_BORDER}` }}>
              ...
              <button onClick={() => startPlan(selected)}
                ...
                style={btn('generate', { width: '100%', padding: '15px', borderRadius: 14, border: 'none', background: PRIMARY, color: 'white', fontSize: 15, fontWeight: 700, fontFamily: 'inherit', marginBottom: 10 }, { background: PRIMARY_DARK })}>
        ...
        {/* Add child form */}
        {adding && (
          <div style={{ background: BEIGE_CARD, borderRadius: 20, padding: 24, border: `2px solid ${BEIGE_BORDER}`, boxShadow: '0 2px 12px rgba(0,0,0,0.06)', animation: 'fadeIn 0.2s ease' }}>
                  ...
                  {['Math', 'Science', 'History', 'Art', 'Music', 'Coding', 'Nature', 'Sports', 'Language', 'Technology'].map(s => {
                    ...
                    return (
                      <button key={s} onClick={() => setForm(p => ({ ...p, subjects: on ? p.subjects.filter(x => x !== s) : [...p.subjects, s] }))}
                        style={{ padding: '6px 14px', borderRadius: 100, border: `2px solid ${on ? PRIMARY : BEIGE_BORDER}`, background: on ? PRIMARY_BG : BEIGE_CARD, color: on ? PRIMARY : TEXT_MUTED, fontSize: 13, fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer' }}>
                        {s}
                      </button>
                    )
                  })}
