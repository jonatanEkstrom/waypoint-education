        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: isMobile ? 10 : 0 }}>
          ...
          {!isMobile && (
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' as const }}>
              {[['children', '👨‍👧 Children', '/dashboard/children'], ['little-readers', '📚 Little Readers', '/little-readers'], ['journal', '📖 Journal', '/journal'], ['community', '🌍 Community', '/community'], ['worksheets', '📄 Worksheets', '/worksheets'], ['la-report', '🏛️ LA Report', '/la-report']].map(([key, label, path]) => (
...
<button onClick={handleLogout}
        ...
        {isMobile && (
          <div style={{ display: 'flex', gap: 6, overflowX: 'auto' as const, paddingBottom: 2 }}>
            {[['children', '👨‍👧', '/dashboard/children'], ['journal', '📖', '/journal'], ['community', '🌍', '/community'], ['worksheets', '📄', '/worksheets'], ['la-report', '🏛️', '/la-report']].map(([key, icon, path]) => (
              <button key={key} onClick={() => router.push(path)}
