...
export default function WorksheetsPage() {
  ...
  const maxScore = worksheet ? worksheet.matching.pairs.length + worksheet.fillblank.sentences.length + worksheet.truefalse.statements.length : 0
        ...
        {worksheet && !loading && (
          <>
            ...
            <div style={s.section}>
              ...
              <button onClick={() => setBlankChecked(true)} style={{ padding: '10px 24px', borderRadius: 100, border: 'none', background: color, color: 'white', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                Check answers ✓
              </button>
            </div>
            ...
            <div style={s.section}>
                  ...
                  <div key={i} style={{ marginBottom: 14 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button onClick={() => !tfChecked && setTfAnswers(prev => ({ ...prev, [i]: true }))}
                        style={{ ...s.btn(userAnswer === true, tfChecked && statement.answer === true, tfChecked && userAnswer === true && !isCorrect), minWidth: 80 }}>
                        👍 True
                      </button>
                      <button onClick={() => !tfChecked && setTfAnswers(prev => ({ ...prev, [i]: false }))}
                        style={{ ...s.btn(userAnswer === false, tfChecked && statement.answer === false, tfChecked && userAnswer === false && !isCorrect), minWidth: 80 }}>
                        👎 False
                      </button>
              ...
              <button onClick={() => setTfChecked(true)} style={{ padding: '10px 24px', borderRadius: 100, border: 'none', background: color, color: 'white', fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', marginTop: 8 }}>
                Check answers ✓
              </button>
            </div>
            ...
            <div style={s.section}>
              ...
              {worksheet.shortanswer.questions.map((q: string, i: number) => (
                <div key={i} style={{ marginBottom: 20 }}>
                  ...
                  <textarea rows={3} style={{ width: '100%', padding: '10px 14px', borderRadius: 12, border: '2px solid #E4E0F5', fontSize: 14, fontFamily: 'inherit', outline: 'none', resize: 'vertical', boxSizing: 'border-box' as const }} placeholder="Write your answer here..." />
                </div>
              ))}
            </div>
            ...
            <div style={{ ...s.section, textAlign: 'center' as const, background: `linear-gradient(135deg, ${color}15, ${color}30)`, border: `2px solid ${color}40` }}>
              <div style={{ fontSize: 32, marginBottom: 8 }}>
                {totalScore() === maxScore ? '🏆' : totalScore() >= maxScore * 0.7 ? '⭐' : '💪'}
              </div>
              <div style={{ fontFamily: 'Georgia,serif', fontSize: 20, color: '#1E1B2E', marginBottom: 4 }}>
                Score: {totalScore()} / {maxScore}
              </div>
              <div style={{ fontSize: 13, color: '#8B87A8' }}>
                {totalScore() === maxScore ? 'Perfect score! Amazing work! 🎉' : totalScore() >= maxScore * 0.7 ? 'Great job! Keep it up! 🌟' : 'Good effort! Try again! 💪'}
              </div>
